"""Deterministic prompt-injection detection for support tickets (L1 regex only)."""
...
class SafetyEngine:
    ...
    _CATEGORY_PATTERNS: dict[str, list[tuple[str, bool]]] = {
        ...
        "FAKE_AUTHORIZATION": [
            ...
            (
                r"\bi have (admin|root|special|elevated) (access|privileges|permission)",
                True,
            ),
        ...
        "OUTPUT_MANIPULATION": [
            (
                r"classify this (ticket|issue|request)? as (replied|escalated|resolved)",
                True,
            ),
        ...
        "DATA_EXFILTRATION": [
            (
                r"show (me )?(all|other|every) (customer|customers|user|users|ticket|tickets|record|records)( tickets)?",
                True,
            ),
    ...
    def scan(self, text: str) -> tuple[bool, str]:
        try:
            ...
            if hit:
                logger.warning("Safety (encoded): %s", reason)
                return True, reason
    ...
    def _check_encoded(self, text: str) -> tuple[bool, str]:
        for token in re.findall(r"[A-Za-z0-9+/]{20,}={0,2}", text):
            ...
            if decoded:
                ...
                hit, _ = self._run_all_checks(normalized_decoded)
                if hit:
    ...
    def _run_all_checks(self, text: str) -> tuple[bool, str]:
        for check in (
            self._check_instruction_override,
            self._check_jailbreak,
            self._check_fake_authorization,
